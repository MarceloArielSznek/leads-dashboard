...
async function fetchOlderLeadsFromAPI(branchId) {
    ...
    try {
        ...
        console.log(`   📅 Looking for leads older than: ${sixtyDaysAgo}`);
        
        while (hasMoreData) {
            // Fetch all leads (not just lost) that are older than 60 days
...
async function updateMatchedLeads() {
    ...
    try {
        ...
        console.log('=== PHASE 1: Current Lost Leads ===');
        ...
        // Fetch older leads from API (60+ days)
        console.log('\n=== PHASE 2: Older Leads (60+ days) ===');
        const allOlderLeads = [];
        const allSoldLeads = [];
        ...
        console.log(`\n📊 Total older leads summary:`);
        console.log(`   📈 ${allSoldLeads.length} sold leads (60+ days old)`);
        console.log(`   📉 ${allOlderLostLeads.length} leads treated as lost (60+ days old)`);
        console.log(`   📊 ${allOlderLeads.length} total older leads`);
        ...
        for (const match of allMatches) {
            ...
            try {
                ...
                const originalStatus = api_lead.original_status ? ` (was: ${api_lead.original_status})` : '';
                
                console.log(`   ${statusIcon} ${leadTypeIcon} ${statusText} (${confidence}): ${db_lead.name} [${db_lead.lead_status}] -> API: ${api_lead.name} [${api_lead.status}]${originalStatus} (${match_type})`);
